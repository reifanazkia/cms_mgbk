    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.getElementById('addForm').addEventListener('submit', function(e) {
                ...
                if (!formData.get('title') || !formData.get('category_kegiatan_id') || !formData.get('image')) {
                ...
                .then(result => {
                    if (result.success) {
                        closeAddModal();
                        showSuccessAlert(result.data.message || 'Kegiatan berhasil ditambahkan!');
                        setTimeout(() => {
                            location.reload();
                        }, 1500);
                    } else {
                        if (result.data.errors) {
                            ...
                            showErrorAlert(result.data.message || 'Gagal menambahkan kegiatan!');
                        }
                    }
                })
                .catch(error => {
            ...
            document.getElementById('editForm').addEventListener('submit', function(e) {
                ...
                // Validate required fields
                if (!formData.get('title') || !formData.get('category_kegiatan_id')) {
                    showErrorAlert('Harap isi semua field yang wajib diisi!');
                    return;
                }
                ...
                .then(response => {
                    ...
                    if (contentType && contentType.includes('application/json')) {
                        ...
                        if (response.ok || response.redirected) {
                            return {
                                success: true,
                                data: { message: 'Kegiatan berhasil diperbarui!' },
                                status: response.status
                            };
                        } else {
                            throw new Error(`HTTP error! status: ${response.status}`);
                ...
                .then(result => {
                    if (result.success) {
                        closeEditModal();
                        showSuccessAlert(result.data.message || 'Kegiatan berhasil diperbarui!');
                        setTimeout(() => {
                            location.reload();
                        }, 1500);
                    } else {
                        if (result.data.errors) {
                            ...
                            showErrorAlert(result.data.message || 'Gagal memperbarui kegiatan!');
                        }
                    }
                })
                .catch(error => {
